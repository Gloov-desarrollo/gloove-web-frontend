...
// --- NUEVO Componente: Efecto de Luz del Cursor ---
const CursorFollower = () => {
    ...
    return <motion.div className="cursor-light" style={{ x: position.x, y: position.y }} />;
};
...
const HeroSection = () => {
    ...
    return (
        <section className="hero-section" style={{backgroundImage: `url('https://images.pexels.com/photos/276724/pexels-photo-276724.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2')`}}>
            <CursorFollower />
...
//--- Componente Genérico para Animación en Scroll ---
...
const ProblemPromiseSection = () => (
    <AnimatedSection className="landing-page-section">
...
const SolutionSection = () => {
    ...
    return (
        <AnimatedSection className="landing-page-section bg-light">
...
const ExclusiveServicesSection = () => {
    const services = [
        { icon: faChartLine, title: "Revenue de Precios", description: "Utilizamos sistemas expertos y análisis de mercado para maximizar cada reserva. Te garantizamos el 100% de optimización para que obtengas siempre el máximo beneficio." },
        { icon: faLightbulb, title: "Asesoramiento Energético", description: "Nuestro equipo de sostenibilidad analiza y optimiza los consumos de tu vivienda, reduciendo costes y aumentando la eficiencia energética para un mayor rendimiento neto." },
        { icon: faPalette, title: "Home Staging y Diseño", description: "Creamos espacios irresistibles. Un equipo especializado en diseño de interiores potencia la estética de tu propiedad para atraer a más huéspedes y mejorar su experiencia." },
        { icon: faBoxOpen, title: "Gestión de Compras y Montaje", description: "Nos encargamos de todo el proceso de puesta a punto: desde la compra y logística del mobiliario hasta el montaje final. Tu vivienda, lista para alquilar sin que te muevas de casa." }
    ];

    return (
        <AnimatedSection className="landing-page-section exclusive-services-section bg-light">
            <div className="container">
...
const ProofSection = () => {
    const testimonials = [
        { quote: "Desde que estoy con Gloove, mi ocupación ha subido un 20% y mis ingresos un 35%. La tranquilidad de la gestión integral no tiene precio.", author: "Carlos Gómez", location: "Apartamento de 2 habitaciones en Altea", img: "https://randomuser.me/api/portraits/men/32.jpg", metrics: ["+35% Ingresos Anuales", "+20% Ocupación"], stars: 5 },
        { quote: "El equipo de Gloove es increíblemente profesional. Se encargan de todo y mi apartamento nunca ha estado en mejores manos. ¡Totalmente recomendados!", author: "Laura Pérez", location: "Estudio en Benidorm", img: "https://randomuser.me/api/portraits/women/44.jpg", metrics: ["+40% Ingresos", "98% Ocupación"], stars: 5 },
        { quote: "La tecnología que utilizan es fantástica. La cerradura inteligente y el panel de propietario me dan una tranquilidad y un control que antes no tenía.", author: "Javier Martínez", location: "Chalet en Calpe", img: "https://randomuser.me/api/portraits/men/46.jpg", metrics: ["+25% Beneficios", "Gestión Simplificada"], stars: 4 }
    ];
    ...
    return (
        <AnimatedSection className="landing-page-section proof-section">
            <div className="container">
                <h2 className="text-center mb-5">Propietarios como tú ya están ganando más con Gloove</h2>
                <Carousel responsive={responsive} infinite autoPlay autoPlaySpeed={5000} keyBoardControl showDots containerClass="testimonial-carousel" dotListClass="custom-dot-list-style" itemClass="testimonial-item">
                    {testimonials.map((testimonial, index) => (
                        <motion.div key={index} className="testimonial-card" whileHover={{ scale: 1.02, boxShadow: "0 15px 35px rgba(0,0,0,0.15)"}}>
...
const OfferSection = () => {
    const plans = [
        { name: "Gestión Esencial", price: "15%", description: "La solución ideal para gestionar tu presencia online y comunicarte con los huéspedes sin esfuerzo.", features: [ { name: "Fotografía Profesional", included: true }, { name: "Creación y Gestión de Anuncios", included: true }, { name: "Publicación en +25 Portales", included: true }, { name: "Atención al Huésped 24/7", included: true }, { name: "Instalación Domótica (Valor 400€)", included: true }, { name: "Panel de Propietario en Tiempo Real", included: true }, { name: "Registro de Viajeros (Policía)", included: true }, { name: "Gestión de Limpieza y Lavandería", included: false }, { name: "Gestión de Mantenimiento", included: false }, ], cta: "Empezar con Esencial" },
        { name: "Gestión Integral", price: "20%", description: "Delega todo en nosotros. La opción perfecta para una tranquilidad total y cero preocupaciones.", features: [ { name: "Fotografía Profesional", included: true }, { name: "Creación y Gestión de Anuncios", included: true }, { name: "Publicación en +25 Portales", included: true }, { name: "Atención al Huésped 24/7", included: true }, { name: "Instalación Domótica (Valor 400€)", included: true }, { name: "Panel de Propietario en Tiempo Real", included: true }, { name: "Registro de Viajeros (Policía)", included: true }, { name: "Gestión de Limpieza y Lavandería", included: true, highlight: "Tranquilidad Total" }, { name: "Gestión de Mantenimiento", included: true, highlight: "Cero Preocupaciones" }, ], cta: "Elegir Gestión Integral", featured: true }
    ];

    return (
        <AnimatedSection className="landing-page-section offer-section">
            <div className="container">
...
const CalculatorSection = () => (
    <AnimatedSection id="calculator" className="landing-page-section bg-light">
       <div className="container">
           <div className="row justify-content-center">
               <div className="col-lg-8 text-center">
                   <h2>Descubre el potencial de tu vivienda en 60 segundos.</h2>
                   <motion.form className="row g-3 mt-3" initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.1 } } }}>
                       <motion.div className="col-md-6" variants={{hidden: { opacity: 0, x: -50 }, visible: { opacity: 1, x: 0 }}}><input type="text" className="form-control" placeholder="Ubicación (Ciudad/Zona)" /></motion.div>
                       <motion.div className="col-md-6" variants={{hidden: { opacity: 0, x: 50 }, visible: { opacity: 1, x: 0 }}}><input type="text" className="form-control" placeholder="Tipo de Propiedad" /></motion.div>
                       <motion.div className="col-md-6" variants={{hidden: { opacity: 0, x: -50 }, visible: { opacity: 1, x: 0 }}}><input type="number" className="form-control" placeholder="Número de Habitaciones" /></motion.div>
                       <motion.div className="col-md-6" variants={{hidden: { opacity: 0, x: 50 }, visible: { opacity: 1, x: 0 }}}><input type="email" className="form-control" placeholder="Tu correo electrónico" /></motion.div>
                       <motion.div className="col-12" variants={{hidden: { opacity: 0, y: 50 }, visible: { opacity: 1, y: 0 }}}><button type="submit" className="btn btn-primary" style={{backgroundColor: '#156B7A', border: 'none', padding: '10px 25px'}}>Calcular mis beneficios ahora</button></motion.div>
                   </motion.form>
               </div>
           </div>
       </div>
...
const FAQItem = ({ q, a }) => {
    ...
    return (
        <div className="faq-item">
            ...
            {isOpen && <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="faq-answer"><p>{a}</p></motion.div>}
        </div>
    );
};
const FAQSection = () => (
...
const FinalPitchSection = () => (
    <AnimatedSection className="landing-page-section text-center" style={{backgroundColor: '#156B7A', color: 'white'}}>
        <div className="container">
